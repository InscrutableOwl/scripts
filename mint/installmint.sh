#! /bin/bash

## flor Linux Mint

bash ~/Documents/mint/packagemint.sh

bash ~/Documents/mint/onepass.sh

bash ~/Documents/mint/brave.sh

bash ~/Documents/mint/dvd.sh

bash ~/Documents/mint/geany.sh

bash ~/Documents/mint/kitty.sh

bash ~/Documents/mint/flatpakmint.sh

bash ~/Documents/mint/move.sh

bash ~/Documents/mint/starship.sh

bash ~/Documents/mint/nord.sh

bash ~/Documents/mint/gitcli.sh

